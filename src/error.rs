...
impl From<JvmFunctionInvokerBuildpackError> for LibCnbError<JvmFunctionInvokerBuildpackError> {
    fn from(error: JvmFunctionInvokerBuildpackError) -> Self {
        LibCnbError::BuildpackError(error)
    }
}

pub struct JvmFunctionInvokerBuildpackErrorHandler {}

impl BuildpackErrorHandle<JvmFunctionInvokerBuildpackError>
    for JvmFunctionInvokerBuildpackErrorHandler
{
    fn handle_error(&self, error: JvmFunctionInvokerBuildpackError) {
        match error {
            JvmFunctionInvokerBuildpackError::OptLayerError(inner) => match inner {
                OptLayerError::CouldNotCopyRunSh(io_error)
                | OptLayerError::CouldNotSetExecutableBitForRunSh(io_error) => {
                    log_error("Unexpected Error")(formatdoc! {"
            ...
            JvmFunctionInvokerBuildpackError::RuntimeLayerError(inner) => match inner {
                RuntimeLayerError::RuntimeDownloadFailed(download_error) => {
                    log_error("Runtime installation failed")(formatdoc! {"
                ...
                RuntimeLayerError::RuntimeChecksumFailed(io_error) => {
                    log_error("Runtime installation failed")(formatdoc! {"
                ...
                RuntimeLayerError::RuntimeChecksumMismatch(checksum) => {
                    log_error("Runtime installation failed")(formatdoc! {"
            ...
            JvmFunctionInvokerBuildpackError::BundleLayerError(inner) => match inner {
                BundleLayerError::NoFunctionsFound => log_error("No functions found")(indoc! {"
                ...
                "}),
                BundleLayerError::MultipleFunctionsFound => {
                    log_error("Multiple functions found")(indoc! {"
                        Your project contains multiple Java functions.
                        Currently, only projects that contain exactly one (1) function are supported.
                    "})
                }
                BundleLayerError::DetectionFailed(exit_code) => {
                    log_error("Detection failed")(formatdoc! {"
                        Function detection failed with internal error \"{exit_code}\"
                    ", exit_code = exit_code })
                }
                BundleLayerError::UnexpectedDetectionTermination => {
                    log_error("Detection failed")(indoc! {"
                        Function detection was unexpectedly terminated without an exit code.
                    "})
                }
                BundleLayerError::BundleCommandIoError(io_error) => {
                    log_error("Detection failed")(formatdoc! {"
                        An unexpected IO error occurred during the detect phase:
                        {io_error}
                    ", io_error = io_error})
                }
                BundleLayerError::CouldNotReadFunctionBundleToml(toml_error) => {
                    log_error("Detection failed")(formatdoc! {"
